...
let generate_cargo_toml_content crate_name dependencies local_crate opam_package_name =
  ...
  (* Shorter eta-expanded helper function with automatic newline *)
  let pf fmt = Printf.bprintf buffer (fmt ^^ "\n") in
  pf "# Generated by %s for %s" program_name opam_package_name;
  pf
    {|
...
let write_crate crate_name dependencies local_crate dune_staticlib_name output_filename opam_package_name =
  ...
  in
  let buffer = Buffer.create 256 in
  Printf.sprintf "; Generated by %s for opam package: %s\n\n" program_name opam_package_name |> Buffer.add_string buffer;
  Printf.sprintf
    {|
...
let gen_staticlib st local_crate_path f opam output_filename =
  ...
  | _ ->
    ...
    let opam_package_name = OpamFile.OPAM.package opam |> OpamPackage.to_string in
    write_crate crate_name crate_deps local_crate dune_staticlib_name output_filename opam_package_name
;;

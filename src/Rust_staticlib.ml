...
let generate_dune_content crate_name dune_staticlib_name =
  ...
  pf " (action";
  pf "  (progn";
  pf "   (run cargo build --release --offline --package %s)" crate_name;
  pf "   (run";
  pf "    mv";
  pf "    %%{workspace_root}/../../target/release/lib%s.a" lib_name;
  pf "    lib%s.a)" lib_name;
  pf "   (run";
  pf "    mv";
  pf "    %%{workspace_root}/../../target/release/lib%s.so" lib_name;
  pf "    dll%s.so))))" lib_name;
  pf "";
  pf "(library";
...
let write_crate crate_directory crate_name dependencies local_crate dune_staticlib_name =
  ...
  |> write_content "Cargo.toml";
  generate_lib_rs_content dependencies local_crate |> write_content "lib.rs";
  generate_dune_content crate_name dune_staticlib_name |> write_content "dune"
;;

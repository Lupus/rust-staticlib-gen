...
let generate_dune_content (crate_name : string) (dune_staticlib_name : string) =
...
let generate_lib_rs_content
  (dependencies : crate_dependency list)
  (local_crate : (string * string) option)
  =
  let buffer = Buffer.create 256 in
  (* Shorter eta-expanded helper function with automatic newline *)
  let pf fmt = Printf.bprintf buffer (fmt ^^ "\n") in
...
let generate_cargo_toml_content
  (crate_name : string)
  (dependencies : crate_dependency list)
  (local_crate : (string * string) option)

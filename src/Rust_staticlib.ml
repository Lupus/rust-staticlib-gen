...
; Bear in mind that two libs like this can not be linked into one binary, as
; static libs produced by Rust expose all Rust stdlib symbols, and linking will
; explode with collision errors.
|};
  pf "";
  pf "(library";
...
let write_crate crate_name dependencies local_crate dune_staticlib_name output_filename =
  ...
  in
  let buffer = Buffer.create 256 in
  Printf.sprintf "; Generated by %s\n\n" program_name |> Buffer.add_string buffer;
  Printf.sprintf
    {|
